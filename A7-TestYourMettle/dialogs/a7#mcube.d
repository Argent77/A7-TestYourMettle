...
IF ~PartyHasItemIdentified("a7#mcube") !AreaType(RUBICON) OR(4) Global("Chapter","GLOBAL",%bg2_chapter_4%) GlobalGT("Chapter","GLOBAL",%bg2_chapter_9%) AreaCheck("AR4000") AreaCheck("AR4500")~ Intro.Denied.1
...
IF ~PartyHasItemIdentified("a7#mcube") !AreaType(RUBICON) Global("Chapter","GLOBAL",%bg2_chapter_7%) Global("EnteredAR2806","GLOBAL",1)~ Intro.Denied.2
  SAY @9001 /* This small metal toy is a replica of a modron, a creature native to the plane of Mechanus. You notice that the item doesn't emit the low humming and buzzing sounds when you try to manipulate its appendages. There may be something in this location interfering with its operation. */
  ++ @9000 /* Put the cube away. */ EXIT
END
