...
IF ~PartyHasItemIdentified("a7#mcube") !Global("A7_RUBIKON","MYAREA",1) OR(4) Global("Chapter","GLOBAL",%bg2_chapter_4%) GlobalGT("Chapter","GLOBAL",%bg2_chapter_9%) AreaCheck("AR4000") AreaCheck("AR4500")~ Intro.Denied.1
...
IF ~PartyHasItemIdentified("a7#mcube") !Global("A7_RUBIKON","MYAREA",1) Global("Chapter","GLOBAL",%bg2_chapter_7%) Global("EnteredAR2806","GLOBAL",1)~ Intro.Denied.2
  SAY @9001 /* This small metal toy is a replica of a modron, a creature native to the plane of Mechanus. You notice that the item doesn't emit the low humming and buzzing sounds when you try to manipulate its appendages. There may be something in this location interfering with its operation. */
  ++ @9000 /* Put the cube away. */ EXIT
END
...
IF ~PartyHasItemIdentified("a7#mcube")~ Intro2
  ...
  + ~!ActuallyInCombat() !Global("A7_RUBIKON","MYAREA",1) Global("A7#KnowCubeRubikon","GLOBAL",0)~
    + @9004 /* Manipulate the toy and look for some sort of effect. */ + Manipulate.Rubikon
  + ~!ActuallyInCombat() !Global("A7_RUBIKON","MYAREA",1) Global("A7#KnowCubeRubikon","GLOBAL",1)~
  ...
  + ~!ActuallyInCombat() Global("A7_RUBIKON","MYAREA",1) Global("A7#LocationRubikon","GLOBAL",1) Global("A7#KnowCubePrime","GLOBAL",0)~
  ...
  + ~!ActuallyInCombat() Global("A7_RUBIKON","MYAREA",1) !AreaCheck("A7#1FD") !AreaCheck("A7#1WZ") Global("A7#LocationRubikon","GLOBAL",1) Global("A7#KnowCubePrime","GLOBAL",1)~
    + @9007 /* Use the combination that activates transport back home. */ + ToPrime
  + ~!ActuallyInCombat() Global("A7_RUBIKON","MYAREA",1) !AreaCheck("A7#1FD") !AreaCheck("A7#1WZ") Global("A7#LocationRubikon","GLOBAL",0)~
    + @9007 /* Use the combination that activates transport back home. */ + ToPrime.Failsafe
  + ~!ActuallyInCombat() Global("A7_RUBIKON","MYAREA",1) OR(2) AreaCheck("A7#1FD") AreaCheck("A7#1WZ") Global("A7#KnowCubePrime","GLOBAL",1) Global("A7#KnowCubeWeakness","GLOBAL",0)~
    + @9007 /* Use the combination that activates transport back home. */ + Transport.Denied
  + ~!ActuallyInCombat() Global("A7_RUBIKON","MYAREA",1) OR(2) AreaCheck("A7#1FD") AreaCheck("A7#1WZ") Global("A7#KnowCubePrime","GLOBAL",1) Global("A7#KnowCubeWeakness","GLOBAL",1)~
    + @9007 /* Use the combination that activates transport back home. */ + Transport.Denied.Known
  + ~ActuallyInCombat() Global("A7_RUBIKON","MYAREA",1) Global("A7#KnowCubePrime","GLOBAL",1)~
    + @9007 /* Use the combination that activates transport back home. */ + Combat.Active
  + ~!ActuallyInCombat() Global("A7_RUBIKON","MYAREA",1) !AreaCheck("A7#1FD") !AreaCheck("A7#1WZ") !AreaCheck("A7#1FY") !AreaCheck("A7#1EN")~
    + @9006 /* Use the combination that activates transport back to the Rubikon dungeon entrance. */ + ToEntrance
  + ~!ActuallyInCombat() Global("A7_RUBIKON","MYAREA",1) OR(2) AreaCheck("A7#1FD") AreaCheck("A7#1WZ") Global("A7#LocationRubikon","GLOBAL",1) Global("A7#KnowCubeWeakness","GLOBAL",0)~
    + @9006 /* Use the combination that activates transport back to the Rubikon dungeon entrance. */ + Transport.Denied
  + ~!ActuallyInCombat() Global("A7_RUBIKON","MYAREA",1) OR(2) AreaCheck("A7#1FD") AreaCheck("A7#1WZ") Global("A7#LocationRubikon","GLOBAL",1) Global("A7#KnowCubeWeakness","GLOBAL",1)~
    + @9006 /* Use the combination that activates transport back to the Rubikon dungeon entrance. */ + Transport.Denied.Known
  + ~ActuallyInCombat() Global("A7_RUBIKON","MYAREA",1)~
